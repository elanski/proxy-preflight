...
def _sanitize_ss_prefix_param(line: str) -> str:
    """
    В некоторых источниках в SS ссылках параметр `prefix=` может содержать
    встроенные control/null байты (например, '\x00') вместо корректного %XX.
    Это делает файлы *_st "бинарными" в редакторах и ломает парсинг.

    Санитизация: если в value после `prefix=` встречаются control/null байты,
    то значение полностью заменяем на percent-encoding по байтам (latin-1),
    сохраняя исходные байты.
    """
    key = "prefix="
    kpos = line.find(key)
    if kpos == -1:
    ...
    # Ищем именно control/null байты; если там уже нормальный текст - не трогаем.
    if not value:
        return line
    if not any((ord(ch) == 0) or (ord(ch) < 32 and ord(ch) not in (9, 10, 13)) for ch in value):
        return line

    try:
        raw = value.encode("latin-1")
    except UnicodeEncodeError:
        # На практике это почти всегда latin-1 совместимо (0..255), но подстрахуемся.
...
def main() -> None:
    ...
    out_lines = [_sanitize_ss_prefix_param(_strip_latency_prefix(item[0])) for item in results]
