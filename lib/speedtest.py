...
from .config import (
    ...
    SPEED_TEST_METRIC,
    SPEED_TEST_MIN_SUCCESS_COUNT,
    SPEED_TEST_MIN_SUCCESS_RATIO,
    SPEED_TEST_MODE,
    SPEED_TEST_REQUESTS,
    SPEED_TEST_TIMEOUT,
    SPEED_TEST_URL,
...
def speed_test_key(
    ...
    try:
        ...
        if not response_times:
            ...
            return None
        success_ratio = (successes / attempted) if attempted > 0 else 0.0
        if successes < SPEED_TEST_MIN_SUCCESS_COUNT or success_ratio < SPEED_TEST_MIN_SUCCESS_RATIO:
            if SPEED_TEST_DEBUG:
                logger.info(
                    "speed_test_key: rejected by success threshold: %s/%s (ratio=%.2f, min_count=%s, min_ratio=%.2f)",
                    successes,
                    attempted,
                    success_ratio,
                    SPEED_TEST_MIN_SUCCESS_COUNT,
